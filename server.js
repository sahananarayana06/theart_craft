...
app.post('/api/reviews', (req, res) => {
  const { productId, name, rating, message } = req.body;
  if (!name || !message) return res.status(400).json({error: 'Name and message are required'});
  const stmt = db.prepare('INSERT INTO reviews (productId,name,rating,message) VALUES (?,?,?,?)');
  stmt.run(productId || null, name, rating || 5, message, function (err) {
    if (err) return res.status(500).json({error: err.message});
